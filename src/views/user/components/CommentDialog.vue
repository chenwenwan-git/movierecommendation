...
<template>
  <el-dialog v-model="dialogVisible" :title="viewOredit ? '修改评论' : '评论详情'" width="30%">
    <div>
      <el-form :model="formModel" ref="form" label-width="100px" style="padding-right: 30px">
        <el-form-item label="电影名称">
          <el-input v-model="formModel.movieName" disabled minlength="1" maxlength="10"></el-input>
        </el-form-item>
        <el-form-item
          v-for="(item, index) in formModel.comment"
          :key="item.commentId"
          :label="`评论${index + 1}`"
        >
          <div style="display: flex; justify-content: space-between; width: 100%">
            ...
            <el-button
              ...
              type="primary"
              @click="change(item.commentId, item.comment)"
            >
              确认修改
            </el-button>
          </div>
